...
object CallFilter {
    ...
    fun judge(call: DeliveryCall, ctx: Context): FilterResult {
        ...
        if (call.isMulti) {
            ...
            if (bundleDist != null && bundleDist > 0) {
                ...
                if (perPrice >= 3000 && distPerItem <= 3.0) {
                    return FilterResult(Verdict.ACCEPT,
                        "우세: 묶음 효율 건당 ${perPriceStr}원 ≥ 3,000원 + 건당 ${"%.1f".format(distPerItem)}km ≤ 3km ($bundleTag$multiPickupTag)")
                }
            }
        ...
        return if (hasDist && unitPrice >= 2500 && call.distance!! <= 3.0) {
            FilterResult(Verdict.ACCEPT,
                "우세: 고단가 근거리 ${fmt.format(unitPrice)}원/km ≥ 2,500원 + 거리 ${"%.1f".format(call.distance)}km ≤ 3km$storeTag$peakTag$directionTag$gpsTag$autoDirectionTag$pointTag")
        } else if (hasDist && unitPrice >= 2000 && call.distance!! <= 2.0) {
            // v3.11: 단거리 고단가 확대 (2,000원/km + 2km 이하)
            FilterResult(Verdict.ACCEPT,
                "우세: 단거리 고단가 ${fmt.format(unitPrice)}원/km ≥ 2,000원 + 거리 ${"%.1f".format(call.distance)}km ≤ 2km$storeTag$peakTag$directionTag$gpsTag$autoDirectionTag$pointTag")
        } else if (hasDist) {
            FilterResult(Verdict.ACCEPT,
            ...
            // 배달거리 없지만 픽업거리 있음 → ACCEPT하되 이유 명시
            FilterResult(Verdict.ACCEPT,
                "금액 ${fmt.format(call.price)}원, 픽업 ${"%.1f".format(pickupKm)}km, 단가 ${fmt.format(unitPrice)}원/km$storeTag$peakTag$directionTag$gpsTag$autoDirectionTag$pointTag")
        } else {
            // 거리 미측정: 금액 기준만 통과
            FilterResult(Verdict.ACCEPT,
    ...
    fun getMinUnitPrice(ctx: Context): Int =
        ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getInt(KEY_MIN_UNIT_PRICE, 1400)
