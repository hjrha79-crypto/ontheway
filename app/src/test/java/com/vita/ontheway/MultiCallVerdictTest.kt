...
/**
 * 멀티콜 verdict 보정 테스트.
 * - 단건 4400원/1.3km → 추천 (단가 3384원/km)
 * - 멀티 4400원/1.3km → 보통 (보정단가 1692원/km, 강등)
 * - 멀티 2000원/1.3km → 멈 (보정단가 769원/km < 1500)
 */
class MultiCallVerdictTest {
    ...
    fun `단건 4400원 1_3km = 추천`() {
        ...
        assertTrue("추천 포함: $msg", msg!!.contains("추천"))
    }
    ...
    fun `멀티 4400원 1_3km = 보통 (강등)`() {
        ...
        assertFalse("추천 미포함: $msg", msg.contains("추천"))
    }

    @Test
    fun `멀티 2000원 1_3km = 멈 (보정단가 미달)`() {
        val call = DeliveryCall(
        ...
        val msg = OutputController.buildMessage(call, acceptResult)
        assertNotNull("메시지가 null이면 안됨", msg)
        assertTrue("멈 포함: $msg", msg!!.contains("멈"))
    }
    ...
    fun `멀티 8000원 2km = 보통 (고액이어도 멀티 강등)`() {
        ...
        val msg = OutputController.buildMessage(call, acceptResult)
        assertNotNull("메시지가 null이면 안됨", msg)
        // 보정단가 = 4000*0.5 = 2000 >= 1500, raw="추천" → 강등 → "보통"
        assertTrue("보통 포함: $msg", msg!!.contains("보통"))
    }
